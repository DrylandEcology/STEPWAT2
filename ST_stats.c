...
void make_header_with_std( char *buf) {
  ...
  fields = (char **)Mem_Calloc(MAX_OUTFIELDS * 2, sizeof(char *), "make_header_with_std", &LogInfo);

  for (i = 0; i < MAX_OUTFIELDS * 2; i++) {
      fields[i] = (char *)Mem_Calloc(MAX_FIELDLEN + 1, sizeof(char), "make_header_with_std", &LogInfo);
  }
...
void make_header( char *buf) {
  ...
  fields = (char **)Mem_Calloc(MAX_OUTFIELDS * 2, sizeof(char *), "make_header", &LogInfo);

  for (i = 0; i < MAX_OUTFIELDS * 2; i++) {
      fields[i] = (char *)Mem_Calloc(MAX_FIELDLEN + 1, sizeof(char), "make_header", &LogInfo);
  }
