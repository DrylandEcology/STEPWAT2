...
FILE * OpenFile(const char *name, const char *mode) {
  ...
  fp=fopen(name, mode);
  if ( isnull(fp) )
    LogError(stdout, LOGERROR|LOGEXIT,
             "Cannot open file %s: %s",name,strerror(errno));
  return(fp);
}
...
char **getfiles(const char *fspec, int *nfound) {
  ...
  Mem_Free(dname); //added these two calls to free to fix a nasty memory leak... since Str_Dup allocates memory with malloc() we free it with free()
  Mem_Free(fname);
