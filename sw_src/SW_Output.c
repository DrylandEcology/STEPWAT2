...
#define OUTSTRLEN 2000 /* max output string length: in get_transp: 4*every soil layer with 14 chars */
...
static void get_none(void);  /* default pfunc until defined */
...
*/
	...
	while( GetALine(f, inbuf) ) {
		...
		x = sscanf(inbuf, "%s %s %s %d %s %s",
		keyname, sumtype, period,
		&first, last, outfile);
		if (x < 6) {
		...
		if (k == eSW_Estab) {
			...
			strcpy(period, "YR");    strcpy(last, "end");
		...
		if (SW_Output[k].use) {
			...
			#ifndef STEPWAT
			SW_Output[k].fp = OpenFile(SW_Output[k].outfile, "w");
			#endif
		}
...
*/
	...
	ForEachOutKey(k) {
		if (SW_Output[k].use)
			CloseFile(&SW_Output[k].fp);
	}
	#endif
}

void SW_OUT_free_outfile_mem(void) {
	// frees mem allocated to SW_Output[k].outfile strings (for use in stepwat)
	OutKey k;
	ForEachOutKey(k) 
		if(SW_Output[k].use)
...
/* --------------------------------------------------- */
	...
	get_outstrleader(pd);
	switch (pd) {
		case eSW_Day:   val = v->dysum.surfaceWater;  break;
		case eSW_Week:  val = v->wkavg.surfaceWater;  break;
		case eSW_Month: val = v->moavg.surfaceWater;  break;
		case eSW_Year:  val = v->yravg.surfaceWater;  break;
	}
	sprintf(str, "%c%7.6f", _Sep, val);
	strcat(outstr, str);
}
...
/* 20110219 (drs) added runoff */
	...
	RealD val_inf = SW_MISSING, val_roff = SW_MISSING;
	char str[OUTSTRLEN];
	
	get_outstrleader(pd);
	switch (pd) {
		case eSW_Day:   val_inf = v->dysum.soil_inf;
						val_roff = v->dysum.runoff;	break;
		case eSW_Week:  val_inf = v->wkavg.soil_inf;
						val_roff = v->wkavg.runoff;	break;
		case eSW_Month: val_inf = v->moavg.soil_inf;
						val_roff = v->moavg.runoff;	break;
		case eSW_Year:  val_inf = v->yravg.soil_inf;
						val_roff = v->yravg.runoff;	break;
	}
	sprintf(str, "%c%7.6f%c%7.6f", _Sep, val_inf, _Sep, val_roff);
	strcat(outstr, str);
}
