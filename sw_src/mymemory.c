...
/*-------------------------------------------
  ...
  cwb - 7/23/2001
 -------------------------------------------*/
  byte *p = (byte *)block,  /* a copy so as not to damage original ? */ // DLM - this isn't an actual copy... it's just copying the address of the pointer (ie shallow copy), you can't deep copy to a pointer that hasn't been allocated
       *pNew;
  #ifdef DEBUG_MEM
    size_t sizeOld;
  #endif
